---
title: "Lab 7: Confidence Intervals for Proportions"
...
The press release for the poll, conducted by WIN-Gallup International, can be accessed [here](https://sidmennt.is/wp-content/uploads/Gallup-International-um-tr%C3%BA-og-tr%C3%BAleysi-2012.pdf).
...
5.  Write out the conditions for inference using the Normal approximation to 
...
6.  What is the margin of error for the estimate of the 
    proportion of the proportion of atheists in US in 2012 as estimated by the 
    normal model? What about as estimated using the bootstrap standard error?

7.  Calculate confidence intervals for the 
    proportion of atheists in 2012 in two other countries of your choice, and 
    report the associated margins of error. You are welcome to use either the normal
    approximation method or the bootstrap, but be sure to note whether the 
...
Lastly, we combine these vectors into a dataframe so that we can visualize it 
either using `geom_point()` or `geom_line()` (or both!).

```{r me-plot}
dd <- data.frame(p = p, me = me)
```

8.  Use ggplot to plot the relationship between `p` and `me` and describe it in words. For
...
knitr::include_app("https://ismay.shinyapps.io/shinyDist/", 
                   height = "600px")
```

9.  Describe the sampling distribution of sample proportions at $n = 300$ and 
    $p = 0.1$. Be sure to note the center, spread, and shape.

10.  Keep $n$ constant and change $p$. How does the shape, center, and spread 
...
11.  Now also change $n$.How does $n$ appear to affect the distribution of $\hat{p}$?
...
13.   Is there convincing evidence that the US has seen a change in its atheism index 
    ...
    You may use either a computational method (`permute` and `bootstrap`) or the 
